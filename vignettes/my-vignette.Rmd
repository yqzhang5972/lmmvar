...
y = X\beta + Zu + e
$$,
where $u_{m\times1} \sim N(0, \sigma_g^2/m)$ and $e_{n\times1} \sim N(0, \sigma_e^2I_n)$ are independent. Let $K = ZZ^T/m$ (kinship matrix), $h^2 = \sigma_g^2 / (\sigma_g^2+\sigma_e^2)$ (heritability) and $\sigma_p^2 = \sigma_g^2+\sigma_e^2$, it then follows that
$$
...
## Example
### Data set up
Let's look at an example based on simulation data. 
```{r}
# -----------------------------------------   
...
There is also a function to find test statistics matrix for $(h^2, \sigma_p^2)^T$ in a predefined grid, the default grid range is from 0 to 1 for both $h^2$ and $\sigma_p^2$. The output of this function is a matrix containing the different test statistics at each grid combination.
```{r message=FALSE}
...
data.frame(p = seq(1e-4,3, length.out = 200)[30:150], teststatistic = cr[100,30:150]) %>%
  ggplot(aes(x = p, y = teststatistic))  + theme_bw() + 
